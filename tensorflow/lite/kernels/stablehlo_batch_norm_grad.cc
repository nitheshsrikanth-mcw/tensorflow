...
TfLiteStatus PrepareTemporaries(TfLiteContext* context, TfLiteNode* node,
  ...
  TfLiteTensor* i6;
  TF_LITE_ENSURE_OK(context, GetTemporarySafe(context, node, /*index=*/5, &i6));
  ...
  TF_LITE_ENSURE_OK(context,
                    GetTemporarySafe(context, node, /*index=*/6,
  ...
  TF_LITE_ENSURE_OK(
      context, GetTemporarySafe(context, node, /*index=*/8, &i3_intermediate));
  ...
  TF_LITE_ENSURE_OK(context, GetTemporarySafe(context, node, /*index=*/9,
    ...
    i6_buffer[i] =
        ((grad_output_buffer[i] *
         ...
         (i3_intermediate_buffer[i % (NumElements(i3_intermediate))] *
                   centered_operand_buffer[i]) / (variance_data[i % (NumElements(variance))] +
                                   static_cast<DataType>(epsilon)));
